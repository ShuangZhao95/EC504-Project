...
csvfilename4 = sys.argv[4] # the combined file for LSH algorithm
#
try :
    f1 = open(csvfilename1,'r')
    f2 = open(csvfilename2,'r')
    f3 = open(csvfilename3,'w')
    f4 = open(csvfilename4,'w')
except:
...
lsh = LocalitySensitiveHashing(
           ...
           dim = D,
           r = 10,                # number of rows in each band for r-wise AND in each band
           b = 50,               # number of bands for b-wise OR over all b bands
           # expected_num_of_clusters = 10,
      )
lsh.get_data_from_csv()
...
for sample_name in similarity_neighborhoods:
    f4.write(str(sample_name)+ ': ' + str(similarity_neighborhoods[sample_name])+'\n')
    n_list = []
    n_class = int(sample_name[6])
    for n_name in similarity_neighborhoods[sample_name]:
        n_list.append(int(n_name[6]))
    if n_list==[]:
        Ctest = -1
    else:
        Ctest = max(set(n_list), key=n_list.count)
    test_mat.append(Ctest)
    f4.write(str(sample_name)+ ': ' + str(Ctest)+'\n')
    if n_class == Ctest:
        Sum_true = Sum_true + 1  
    i = i + 1  
f4.close()
print("CCR is " + str(Sum_true/i))
##for basic NN problem
# similarity_groups = lsh.lsh_basic_for_neighborhood_clusters()
# coalesced_similarity_groups = lsh.merge_similarity_groups_with_coalescence( similarity_groups )
#
# merged_similarity_groups = lsh.merge_similarity_groups_with_l2norm_set_based( coalesced_similarity_groups )
